...
- **Home** (`index.html`): hero (cream) → **Research as Performance** (**cream band**, distilled/dramatic/targeted/timely) → **What I've built** (**forest/green band**: Saga / Aria / Corpus trio + "See the work" link to portfolio) → footer. The **About section was removed** (2026) — its content (foundational-questions framing, the Talent Insights → builder-turn line, the "DESIGN → RESEARCH → BUILD" card) is not preserved elsewhere; the hero subtitle is now the only place carrying the design-background + builder narrative. What I've built is the only green band on Home; the Research-as-Performance section's own top/bottom borders (`#E7DECB`) separate it from hero and from What I've built, so no additional separator is needed.
